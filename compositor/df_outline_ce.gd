...
const EXTRACTION_SHADER_PATH := "res://outlines/df_outline_ce/shaders/extraction.glsl"
const JF_PASS_SHADER_PATH := "res://outlines/df_outline_ce/shaders/jf_pass.glsl"
const OVERLAY_SHADER_PATH := "res://outlines/df_outline_ce/shaders/overlay.glsl"
...
const NOISE_IMAGE_BINDING := 4

# Set 3 bindings
const LIGHT_DIRECTION_BINDING := 0
...
var noise_image_uniform : RDUniform

var df_data_ubo : RID
var df_data_ubo_uniform : RDUniform

var light_direction_ubo : RID
var light_direction_uniform : RDUniform
...
func _render_view(p_view : int) -> void:
	...
	create_light_direction_buffer()
	var light_direction_uniform_set : Array[RDUniform] = [light_direction_uniform]
	if USE_DEBUG_IMAGE:
	...
	uniform_sets = [
		scene_uniform_set,
		df_uniform_set,
		[texture_a_out_image_uniform, depth_image_uniform, noise_image_uniform],
		light_direction_uniform_set
	]
...
func setup_outline_settings() -> void:
	...
	if not outline_settings.blink_amount_changed.is_connected(update_blink_amount):
		outline_settings.blink_amount_changed.connect(update_blink_amount)
	if not outline_settings.bulge_amount_changed.is_connected(update_bulge_amount):
		outline_settings.bulge_amount_changed.connect(update_bulge_amount)
	update_jf_calcs()


func update_blink_amount() -> void:
	settings_dirty = true
	
func update_bulge_amount() -> void:
	settings_dirty = true
...
func create_global_uniform_buffer() -> void:
	...
	var data : Array = [
			...
			outline_settings.nearby_depth_search_distance, # float
			outline_settings.noise_scale, # float
			
			outline_settings.noise_intensity, # float
			outline_settings.noise_wiggle_speed, # float
			...
			outline_settings.blink_amount, # float
			outline_settings.bulge_amount # float
			# unused float
	]
...
func create_light_direction_buffer() -> void:
	if light_direction_ubo.is_valid():
		free_rid(light_direction_ubo)
	
	var data : Array = [
			light_direction, #vec4 // but w is unused. Can only pass vec4 though
	]
	
	light_direction_ubo = create_uniform_buffer(data)
	light_direction_uniform = get_uniform_buffer_uniform(light_direction_ubo, LIGHT_DIRECTION_BINDING)
	settings_dirty = false
...
func create_textures() -> void:
	...
	#var image := outline_settings.noise_texture.get_image()
	var image_file : Texture2D = load("res://textures/noise_tex_3.png")
	...
	noise_image_uniform = get_sampler_uniform(tex, sampler, NOISE_IMAGE_BINDING)
